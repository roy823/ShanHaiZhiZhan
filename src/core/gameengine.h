...
public:
    ...
    // 获取可用精灵
    QVector<Creature*> getAvailableCreatures() const;
    void addAvailableCreature(Creature* creature);
    void removeAvailableCreature(int index);
    void clearAvailableCreatures();
    ...
    void setBattlesWon(int value);
    void setBattlesLost(int value);
      // 启动战斗
    ...
    Creature* createCreature(const QString& creatureName, const Type& type, int level = 1);
      // 创建技能实例
    Skill* createSkill(const QString& skillName, ElementType elementType, SkillCategory skillCategory, 
...
private:
    ...
    // 可用精灵列表
    QVector<Creature*> m_availableCreatures;
