...
void CreatureDetailWidget::updateCreatureInfo(Creature* creature) {
    ...
    QString statsText = QString(
     ...
     .arg(stats.attack).arg(stats.defense)
     .arg(stats.specialAttack).arg(stats.specialDefense)
     .arg(stats.speed);
    ...
    for (int i = 0; i < creature->getSkillCount(); ++i) {
        ...
        QString skillText = QString("%1 (%2, %3) PP: %4/%5")
            .arg(skill->getName())
            .arg(Type::getElementTypeName(skill->getElementType()))
            .arg(skill->getSkillType() == SkillType::PHYSICAL ? "物理" : 
                 skill->getSkillType() == SkillType::SPECIAL ? "特殊" : "状态")
        ...
        // 根据属性设置颜色
        QString typeColor = Type::getElementTypeColor(skill->getElementType());
        item->setForeground(QColor(typeColor));
