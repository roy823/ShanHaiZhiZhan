                                                                    ...
                                                                    m_animationTimer(nullptr)
{
    ...
    connect(m_battleSystem, &BattleSystem::battleEnded, this, [this](BattleResult result)
    {
        ...
        // 根据战斗结果设置消息文本
        switch (result) {
            case BattleResult::PLAYER_WIN: resultMessage = "<b style='color:green'>你赢了!</b>"; break;
            case BattleResult::OPPONENT_WIN: resultMessage = "<b style='color:red'>你输了!</b>"; break;
            case BattleResult::DRAW: resultMessage = "<b style='color:orange'>平局!</b>"; break;
            case BattleResult::ESCAPE: resultMessage = "<b style='color:blue'>成功逃脱!</b>"; break;
            default: resultMessage = "<b>战斗结束</b>";
        }
        updateBattleLog(resultMessage); // 在日志中显示结果
        disableAllActionButtons();      // 战斗结束，禁用所有行动按钮
        
        // 添加最终消息提示自动返回
        updateBattleLog("<i>3秒后自动返回主界面...</i>");
        
        // 使用计时器设置延迟，让玩家有时间看到战斗结果
        QTimer::singleShot(3000, this, [this]() {
            if (m_gameEngine) {
                emit battleFinished(); // 发出战斗结束信号
                // 或者直接调用:
                // m_gameEngine->endBattle();  // 通知游戏引擎战斗结束
            }
        });
    });
