...
bool Skill::use(Creature *user, Creature *target, BattleSystem *battle)
{
    ...
    if (m_category == SkillCategory::STATUS && !target) { // 对于一些以自身为目标的状态技能
        ...
        for (Effect *effect : m_effects) {
            if (effect) {
                effect->apply(user, target, battle); // 让 Effect 内部自己处理概率
            }
        }
...
bool MultiHitSkill::use(Creature *user, Creature *target, BattleSystem *battle)
{
    ...
    for (int i = 0; i < numberOfHits; ++i) { // 至少检查一次命中
        if (checkHit(user, target, battle)) {
            ...
            // 示例：每次命中都尝试触发效果
            for (Effect *effect : m_effects) {
                if (effect && effect->checkChance()) { // 效果自身也有触发几率
...
FifthSkill::FifthSkill(const QString &name, ElementType type, SkillCategory category,
                       int power, int ppCost, int accuracy, int priority)
    : Skill(name, type, category, power, ppCost, accuracy, priority) // 调用基类 Skill 的构造函数
{
    // 中文注释：第五技能基类构造函数
    // 第五技能的特定初始化（如果需要）
    // 例如，可以设置一个标志表明这是一个第五技能，或者第五技能有统一的PP消耗上限等。
}
...
{
    // 中文注释：检查第五技能使用条件
    // target_unused 和 battle_unused 标记为可能未使用，具体看条件。
    // 如果某些第五技能的使用条件与目标或战斗状态有关，则需要使用这些参数。

    if (!user) { // 使用者必须存在
        return false;
    }

    // 检查基本的PP消耗 (虽然实际使用时BattleSystem或Creature::useSkill也会检查)
    // if (user->getCurrentPP() < getPPCost()) {
    //     return false;
    // }

    // 示例：根据技能名称判断特定条件
    // 这个判断逻辑也可以放在具体的派生第五技能类中（如果每个第五技能都是一个新类）
    // 或者，如果第五技能都继承自FifthSkill，通过名称来区分特殊逻辑。
    if (m_name == "不屈战魂") {
        // 条件：使用者HP低于最大HP的一半
        return user->getCurrentHP() < (user->getMaxHP() / 2);
    }
    // 示例：另一个第五技能可能有不同的条件
    // else if (m_name == "另一个第五技能的名称") {
    //    // 检查其他条件，可能需要 battle 或 target 参数
    //    // return (battle && battle->getWeather() == WeatherType::SUNNY); // 举例
    // }


    return true; // 默认情况下，如果没有其他特定条件，则第五技能可用 (前提是PP足够)
}