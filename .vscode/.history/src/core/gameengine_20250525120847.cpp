...
Creature* GameEngine::createCreature(const QString& creatureName, const Type& type, int level) {
    ...
    // 简单实现示例
    BaseStats baseStats;
    baseStats.hp = 50 + level * 5;
    baseStats.attack = 40 + level * 3;
    baseStats.defense = 40 + level * 3;
    baseStats.specialAttack = 40 + level * 3;
    baseStats.specialDefense = 40 + level * 3;
    baseStats.speed = 40 + level * 3;
...
// 创建技能
Skill* GameEngine::createSkill(const QString& skillName, ElementType elementType, SkillType skillType,
