...
PrimalShiftSkill::PrimalShiftSkill() 
    : StatusSkill("狂化变身", ElementType::NORMAL, 4, 100)
{
}

bool PrimalShiftSkill::use(Creature *user, Creature *target, BattleSystem *battle)
{
    // 中文注释：使用狂化变身技能
    // 调用基类的use进行PP消耗和基础判定
    if (!StatusSkill::use(user, target, battle))
        return false;

    if (ChimpanziniBananini *chimp = dynamic_cast<ChimpanziniBananini *>(user))
    {
        chimp->enterBerserkForm(3); // 3回合持续时间
        // if (battle) battle->addBattleLog(QString("%1 进入了狂暴形态!").arg(user->getName()));
    }
    else
    {
...
QString PrimalShiftSkill::getDescription() const
{
    return "进入
    "狂暴形态"：物攻等级+2，速度等级+1，物防等级-1，特防等级-1。此效果持续3回合，结束后恢复原形态和能力等级。";
}
