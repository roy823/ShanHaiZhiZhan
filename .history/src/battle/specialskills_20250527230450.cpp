...
bool FifthSkill::use(Creature *user, Creature *target, BattleSystem* battle)
{
    ...
    if (!canUse(user, target, battle)) {
        if (battle) {
            battle->addBattleLog(QString("%1 无法使用 %2！条件不满足。").arg(user->getName()).arg(Skill::getName()));
        }
        return false;
    }
    ...
    if (Skill::m_category == SkillCategory::STATUS && !actualTarget && !Skill::getEffects().isEmpty() && Skill::getEffects().first()->isTargetSelf()) {
        actualTarget = user;
    }

    // 命中检查
    if (Skill::checkHit(user, actualTarget, battle)) {
        // 应用所有效果
