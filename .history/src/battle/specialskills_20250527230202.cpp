...
bool FifthSkill::use(Creature *user, Creature *target, BattleSystem* battle)
{
    ...
    if (!canUse(user, target, battle)) {
        if (battle) {
            battle->addBattleLog(QString("%1 无法使用 %2！条件不满足。").arg(user->getName()).arg(getName()));
        }
        return false;
    }
    ...
    if (m_category == SkillCategory::STATUS && !actualTarget && !getEffects().isEmpty() && getEffects().first()->isTargetSelf()) {
        actualTarget = user;
    }
