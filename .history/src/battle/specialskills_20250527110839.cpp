    ...
    : FifthSkill("不屈战魂", ElementType::NORMAL, SkillCategory::STATUS, 0, 3, 100)
{
    ...
    // 添加效果时使用新的目标类型
    addEffect(new StatChangeEffect(StatType::ATTACK, 2, TargetType::SELF));
    addEffect(new StatChangeEffect(StatType::SPEED, 1, TargetType::SELF));
    ...
    : FifthSkill("空域压制", ElementType::FLYING, SkillCategory::STATUS, 0, 3, 100)
{
    ...
    auto airspaceLambda = [](Creature *affectedCreature, Creature *sourceCreature, BattleSystem *battle, TurnBasedEffect *self)
    {
        if (affectedCreature)
        {
    ...
    debuffEffect->setTargetType(TargetType::OPPONENT);
    ...
    : FifthSkill("生命汲取领域", ElementType::GRASS, SkillCategory::STATUS, 0, 4, 100)
{
    ...
    auto siphonLambda = [](Creature *affectedCreature, Creature *sourceCreature, BattleSystem *battle, TurnBasedEffect *self)
    {
        if (!battle || !affectedCreature || !sourceCreature)
            return;
        
        // 获取对手
        Creature *opponent = battle->getOpponentActiveCreature() == sourceCreature ? 
        ...
        // 治疗自身
        sourceCreature->heal(sourceCreature->getMaxHP() / 8);
    };
    
    TurnBasedEffect *siphonEffect = new TurnBasedEffect(3, siphonLambda, false);
    siphonEffect->setTargetType(TargetType::SELF);
