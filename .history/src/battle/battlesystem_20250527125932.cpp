...
void BattleSystem::triggerHealingReceived(Creature* creature, int amount, const QString& source) {
    if (amount <= 0) return;
    
    if (!source.isEmpty()) {
        addBattleLog(QString("%1 通过%2恢复了%3点生命值!").arg(creature->getName()).arg(source).arg(amount), nullptr, creature);
    } else {
...
void BattleSystem::triggerDamageCaused(Creature* creature, int amount, const QString& source) {
    if (amount <= 0) return;
    
    // 可以根据来源添加不同的日志
    if (!source.isEmpty()) {
        addBattleLog(QString("%1 受到了%2造成的%3点伤害!").arg(creature->getName()).arg(source).arg(amount), nullptr, creature);
    } else {
...
void BattleSystem::onStatStageChanged(Creature* target, StatType stat, int oldStage, int newStage) 
{
    if (!target) return;
    
    QString statName = StatStages::statTypeToString(stat);
