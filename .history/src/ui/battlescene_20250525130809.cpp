...
QString BattleScene::getStatStageText(const StatStages& stages) {
    ...
    if (stages.getStage(StatType::ATTACK) != 0) {
        changes << QString("攻击: %1%2").arg(stages.getStage(StatType::ATTACK) > 0 ? "+" : "").arg(stages.getStage(StatType::ATTACK));
    }
    if (stages.getStage(StatType::DEFENSE) != 0) {
        changes << QString("防御: %1%2").arg(stages.getStage(StatType::DEFENSE) > 0 ? "+" : "").arg(stages.getStage(StatType::DEFENSE));
    }
    if (stages.getStage(StatType::SP_ATTACK) != 0) {
        changes << QString("特攻: %1%2").arg(stages.getStage(StatType::SP_ATTACK) > 0 ? "+" : "").arg(stages.getStage(StatType::SP_ATTACK));
    }
    if (stages.getStage(StatType::SP_DEFENSE) != 0) {
        changes << QString("特防: %1%2").arg(stages.getStage(StatType::SP_DEFENSE) > 0 ? "+" : "").arg(stages.getStage(StatType::SP_DEFENSE));
    }
    if (stages.getStage(StatType::SPEED) != 0) {
        changes << QString("速度: %1%2").arg(stages.getStage(StatType::SPEED) > 0 ? "+" : "").arg(stages.getStage(StatType::SPEED));
    }
    if (stages.getStage(StatType::ACCURACY) != 0) {
        changes << QString("命中: %1%2").arg(stages.getStage(StatType::ACCURACY) > 0 ? "+" : "").arg(stages.getStage(StatType::ACCURACY));
    }
    if (stages.getStage(StatType::EVASION) != 0) {
        changes << QString("闪避: %1%2").arg(stages.getStage(StatType::EVASION) > 0 ? "+" : "").arg(stages.getStage(StatType::EVASION));
    }
