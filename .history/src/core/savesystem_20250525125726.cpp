...
Creature* SaveSystem::createCreatureFromJson(const QJsonObject& json) const {
    ...
    for (const QJsonValue& skillValue : skillsArray) {
        ...
        SkillCategory skillcategory = static_cast<SkillCategory>(skillObject["skillCategory"].toInt());
        ...
        Skill* skill = gameEngine->createSkill(skillName, elementType, skillCategory, power, accuracy, maxPP);
        if (skill) {
            if (currentPP < maxPP) {
                skill->consumePP(maxPP - currentPP);
            }
