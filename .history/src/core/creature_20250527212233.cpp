...
void Creature::onTurnStart(BattleSystem* battle)
{
    ...
    switch (m_statusCondition) {
        case StatusCondition::PARALYZE:
            if (QRandomGenerator::global()->bounded(100) < 25) { // 25%概率无法行动
                m_canActThisTurn = false;
                if (battle) battle->addBattleLog(QString("%1 因麻痹无法行动!").arg(m_name));
            }
            break;
        case StatusCondition::SLEEP:
            m_canActThisTurn = false;
            if (battle) battle->addBattleLog(QString("%1 正在睡眠中，无法行动!").arg(m_name));
            // 有几率自动醒来
            if (QRandomGenerator::global()->bounded(100) < 33) { // 33%概率醒来
                clearStatusCondition();
                if (battle) battle->triggerStatusChanged(this, StatusCondition::SLEEP, StatusCondition::NONE);
            }
            break;
        case StatusCondition::FREEZE:
            m_canActThisTurn = false;
            if (battle) battle->addBattleLog(QString("%1 被冻伤了，无法行动!").arg(m_name));
            // 冻伤每回合扣血
            takeDamage(m_maxHP / 8); // 每回合损失最大HP的1/8
            break;
    }
