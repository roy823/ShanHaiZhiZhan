...
// 处理回合结束时的状态效果
    ...
    switch (m_statusCondition) {
        case StatusCondition::BURN:
            ...
            takeDamage(damage);
            if (battle) battle->addBattleLog(QString("%1 因烧伤受到了 %2 点伤害!").arg(m_name).arg(damage));
            break;
        case StatusCondition::POISON:
            damage = m_maxHP / 8; // 中毒每回合损失1/8最大生命值
            takeDamage(damage);
            if (battle) battle->addBattleLog(QString("%1 因中毒受到了 %2 点伤害!").arg(m_name).arg(damage));
            break;
        case StatusCondition::BLEED:
            damage = m_maxHP / 10; // 流血每回合损失1/10最大生命值
            takeDamage(damage);
            if (battle) battle->addBattleLog(QString("%1 因流血受到了 %2 点伤害!").arg(m_name).arg(damage));
            break;
        // 其他状态效果...
    }
...
void LiriliLarila::onTurnStart(BattleSystem* battle) { Creature::onTurnStart(); }
void LiriliLarila::onTurnEnd(BattleSystem* battle) { Creature::onTurnEnd(); }
